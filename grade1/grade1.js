/* Grade 1 Activities Page JavaScript - Meadow Math */

/**
 * Grade 1 page controller
...
const Grade1Page = {
  ...
  async loadData() {
    try {
      ...
      console.error('Error loading Grade 1 data:', error);
      this.renderError();
    }
  },
  ...
  renderLearnMoreText(level) {
    ...
    const resourcesHtml = level.learnMore.resources.map((resource, index) => {
      ...
      if (resource.source === 'youtube') {
        ...
        return `
          <li>
            <a href="${searchUrl}" target="_blank" rel="noopener noreferrer" class="youtube-search-link">${this.escapeHtml(query)}</a>: ${this.escapeHtml(description)}
          </li>
        `;
      } else {
